	...
	export default {
		...
		methods: {
			...
			publishBtn() {
				...
				// 获取文章主体html元素
				let mainDomList = this.$refs.inputDiv.children
				let mainStr = ""
				let mainInfo = ""
				for (let i = 0; i < mainDomList.length; i++) {
					console.log(mainDomList[i]);
					// 删除dom元素中可编辑属性
					mainDomList[i].removeAttribute("contenteditable")
					if (mainDomList[i].tagName == "P") {
						mainInfo += mainDomList[i].innerText
						if (mainDomList[i].children.length != 0) {
							for (let j = 0; j < mainDomList[i].children.length; j++) {
								if (mainDomList[i].children[j].tagName == "SPAN") {
									mainDomList[i].children[j].removeAttribute("contenteditable")
								}
							}
						}
					}
				...
				this.$refs["uploadFormRef"].validate((valid) => {
					if (valid) {
						http.post("/api/api/uploadArticle", {
							...
							coverPath: this.uploadForm.imageUrl.split("?")[0],
							openId: this.openId,
							info: mainInfo
						}).then(res => {
